...
public:
	...
	bool operator<(const Edge& e) {
		return (weight < e.weight && isWeighted == e.isWeighted);
	}
...
public:
	...
	bool isDirected(); // является ли ориентированным
	bool isWeighted(); // является ли взвешенным

	void addEdge(int f, int s, int w = 0); // добавляет ребро
	void addVertex(int s); // добавляет вершину
	void deleteVertex(int v); // удаляет вершину
...
private:
	...
	bool weighted;
	/* вершина - вектор пар,
	где first - смежная с ней вершина,
	second - вес ребра */
